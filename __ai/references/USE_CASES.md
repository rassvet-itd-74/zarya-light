...
Observable behavior the client must exhibit. Each row is a test target.

Sections are referenced by name, not by letter, and rows are numbered within their section. Inserting a section must never renumber another one.
...
| 9–10 | Cast `FOR` / `AGAINST` | `castVote(votingId, support, organ)` |
| 11 | Transfer chairmanship | Explicitly privileged; never routine or automatic |
| 12–13 | Set organ quorum / approval | `setMinimumQuorum`, `setMinimumApprovalPercentage` |
...
- Rows 9–10: authorization for a vote is membership in **the voting's own organ**, which the contract reads from stored state — `castVote` takes no organ argument, so no form supplies one. The Chairman is accepted in any organ for `castVote` and membership votings, and rejected without membership on the four matrix-configuration votings. Theme and statement votings have no organ and accept any address.
- Rows 12–13: signer verified as Chairman where possible, never generated by the executor, and a later threshold change does not retroactively alter an existing voting. Percentage unit and base come from the contract; `51` is not reinterpreted.
