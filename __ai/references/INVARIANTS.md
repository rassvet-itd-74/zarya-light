...
- **The member wallet is generated by the application, not configured.** A key in an environment variable is a key in a plaintext file that a backup, a screen share, or a stray `git add` eventually copies. `MemberKeyStore` creates one on first start, encrypts it with `safeStorage`, and stores it beside the database. It is created **exactly once** — replacing it would strand every operation already sent from the old address.
- **No encryption available means no wallet**, not a wallet in the clear. A member who cannot sign is in a better position than one whose key sits in a readable file they do not know about.
- **The key crosses one process boundary, in one message.** `safeStorage` is a main-process API — Electron declares it in `Main` and not in `Utility` — so main decrypts and hands the key to the worker at each worker start. Not through the environment: an environment is inherited by any child process and readable from outside on several platforms.
- **There is no backup, and that is an open product decision.** The encryption is bound to the operating-system account, so a reinstalled machine or a lost profile makes the wallet unrecoverable — which for a governance wallet means an address that can never act again. Either an export path or an accepted policy of re-registering a new address is owed before this reaches members.
